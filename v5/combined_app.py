...
async def post_chat(chat: ChatRequest):
    try:
        ...
        current_system_prompt = state["messages"][0]["content"]
        ...
        if state["medi_ai_mode"]:
            # Toggle ON: Chat responses come only from the Qdrant vector database (retrieval-based only).
            logger.info("Toggle ON: Responding only from MediAI (Qdrant-based) mode.")
            ...
            try:
                previous_message = truncated_history[-1]["content"] if truncated_history else ""
                ...
                if context and "No relevant information" not in context:
                    ...
                    final_answer = "I am unable to find relevant information in the current medical eBook sources. Please try rephrasing your question or consult a healthcare professional for specific medical advice."
            except Exception as search_error:
                logger.error(f"Error searching Qdrant: {str(search_error)}")
                final_answer = f"Error searching Qdrant: {str(search_error)}"
            
            if not final_answer:
                try:
                    response = call_ollama_with_retry(messages_to_send)
                    final_answer = response.choices[0].message.content
                    if "No relevant information" not in context and "[" not in final_answer and "]" not in final_answer:
                    ...
                    final_answer = f"Error generating Qdrant-only response: {str(e)}"

        else:
            # Default mode: MediAI and Mistral responses
            logger.info("MediAI mode is OFF. Responding from MediAI and Mistral models.")
            
            # MediAI response (with Qdrant context from DEFAULT_COLLECTION)
            medi_ai_answer = ""
            try:
                system_prompt_medi_ai = MEDICAL_PROMPT if state["medical_mode"] else GENERAL_PROMPT
                ...
                if context and "No relevant information" not in context:
                    ...
                    logger.info("No relevant content found in default database for MediAI")
                
                response_medi_ai = call_ollama_with_retry(messages_to_send_medi_ai)
                medi_ai_answer = response_medi_ai.choices[0].message.content
                if state["medical_mode"] and "No relevant information" not in context and "[" not in medi_ai_answer and "]" not in medi_ai_answer:
                ...
                logger.error(f"Error generating MediAI response: {str(e)}")
                medi_ai_answer = f"Error generating MediAI response: {str(e)}"

            # Mistral response (without Qdrant context, general prompt)
            mistral_answer = ""
            try:
                ...
                mistral_answer = response_mistral.choices[0].message.content
            except Exception as e:
                logger.error(f"Error generating Mistral response: {str(e)}")
                mistral_answer = f"Error generating Mistral response: {str(e)}"
            
            final_answer = f"**MediAI Response:**\n{medi_ai_answer}\n\n**Mistral Response:**\n{mistral_answer}"
