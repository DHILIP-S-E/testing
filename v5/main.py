...
async def get_chat():
    ...
    return {
        ...
        "medical_mode": state["medical_mode"],
        "medi_ai_mode": state["medi_ai_mode"],
        "active_mode": "medi_ai" if state["medi_ai_mode"] else "common"
    }
...
async def post_chat(chat: ChatRequest):
    try:
        logger.info(f"Current medi_ai_mode state: {state['medi_ai_mode']}")
        ...
        # This keeps the most recent messages
        truncated_history = conversation_history[-CONFIG["MAX_CHAT_HISTORY_FOR_MODEL"]:]

        # Generate both medi_ai and common responses regardless of the toggle state

        # 1. Generate medi_ai response (Qdrant-only)
        medi_ai_answer = ""
        try:
            logger.info("Generating MediAI (Qdrant-based) response.")
            system_prompt = MEDICAL_PROMPT # Qdrant books are medical
            messages_to_send = [{"role": "system", "content": system_prompt}] + truncated_history

            previous_message = truncated_history[-2]["content"] if len(truncated_history) > 1 else ""
            search_query = f"{previous_message} {chat.message}"
            context = search_qdrant(search_query, collection_name=CONFIG["DEFAULT_COLLECTION"])
            if context and "No relevant information" not in context:
                ...
                medi_ai_answer = response.choices[0].message.content
                if "[" not in medi_ai_answer and "]" not in medi_ai_answer:
                ...
                logger.info("No relevant content found in the Qdrant vector database")
                medi_ai_answer = "I am unable to find relevant information in the current medical eBook sources. Please try rephrasing your question or consult a healthcare professional for specific medical advice."
        except Exception as e:
            logger.error(f"Error generating Qdrant-only response: {str(e)}")
            medi_ai_answer = f"Error generating Qdrant-only response: {str(e)}"

        # 2. Generate common response (hybrid of Qdrant and Mistral)
        common_answer = ""
        try:
            logger.info("Generating Common (hybrid) response.")

            # First part: MediAI with Qdrant context
            medi_ai_part = ""
            try:
                system_prompt_medi_ai = MEDICAL_PROMPT
                messages_to_send_medi_ai = [{"role": "system", "content": system_prompt_medi_ai}] + truncated_history

                context = search_qdrant(chat.message, collection_name=CONFIG["DEFAULT_COLLECTION"])
                ...
                if context and "No relevant information" not in context:
                    ...
                    """
                    messages_to_send_medi_ai.insert(1, {"role": "system", "content": context_instruction})
                    citation_example = """
                    Example of proper citation format:
                    ...
                    """
                    messages_to_send_medi_ai.insert(2, {"role": "system", "content": citation_example})

                response_medi_ai = call_ollama_with_retry(messages_to_send_medi_ai)
                medi_ai_part = response_medi_ai.choices[0].message.content
                if "No relevant information" not in context and "[" not in medi_ai_part and "]" not in medi_ai_part:
                ...
                logger.error(f"Error generating MediAI part of hybrid response: {str(e)}")
                medi_ai_part = f"Error generating MediAI part: {str(e)}"

            # Second part: Mistral response (without Qdrant context)
            mistral_part = ""
            try:
                system_prompt_mistral = GENERAL_PROMPT # Mistral is general purpose
                messages_to_send_mistral = [{"role": "system", "content": system_prompt_mistral}] + truncated_history
                response_mistral = call_ollama_with_retry(messages_to_send_mistral)
                mistral_part = response_mistral.choices[0].message.content
            except Exception as e:
                logger.error(f"Error generating Mistral part of hybrid response: {str(e)}")
                mistral_part = f"Error generating Mistral part: {str(e)}"

            common_answer = f"**MediAI Response:**\n{medi_ai_part}\n\n**Mistral Response:**\n{mistral_part}"
        except Exception as e:
            logger.error(f"Error generating hybrid response: {str(e)}")
            common_answer = f"Error generating hybrid response: {str(e)}"

        # Determine which response to show in the chat history based on the toggle state
        display_answer = medi_ai_answer if state["medi_ai_mode"] else common_answer

        # Store the display answer in the chat history
        state["messages"].append({"role": "assistant", "content": display_answer})

        # Return both responses in a unified JSON
        return {
            "messages": [m for m in state["messages"] if m["role"] != "system"],
            "responses": {
                "medi_ai": medi_ai_answer,
                "common": common_answer
            },
            "active_mode": "medi_ai" if state["medi_ai_mode"] else "common"
        }
    except Exception as e:
...
async def new_session():
    ...
    # Start a new session with a new UUID
    ...
    try:
        ...
        for session in history:
            if session["session_id"] == session_id:
                state["medical_mode"] = session.get("medical_mode", False)
                state["medi_ai_mode"] = session.get("medi_ai_mode", False)
