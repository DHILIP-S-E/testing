...
def query_qdrant_for_book(book_name: str, limit: int = 20):
    ...
    try:
        ...
        for collection_name in collection_names:
            logger.info(f"Querying collection: {collection_name} for book/topic: {book_name}")
            try:
                # Get collection info
                collection_info = qdrant_client.get_collection(collection_name)
                logger.info(f"Collection {collection_name} has {collection_info.vectors_count} vectors")
                ...
                if not search_result:
                    logger.info(f"No exact match for {book_name} in {collection_name}, searching without filter")
                    search_result = qdrant_client.search(
                        collection_name=collection_name,
                        query_vector=query_embedding,
                        limit=limit,
        ...
        logger.info(f"Found {len(all_results)} chunks for {book_name} across all collections")
        return all_results
    except Exception as e:
...
async def generate_exam(exam_request: ExamRequest):
    ...
    # For Medi AI, we fetch content from Qdrant
        ...
        if not book_chunks:
            raise HTTPException(
                status_code=404,
                detail=f"No content found for book '{exam_request.book_name}' in any collection."
            )
    ...
    # Try up to 5 times to get the requested number of questions
    max_retries = 5
    retry_count = 0
    ...
    while len(mcq_questions) < exam_request.total_questions and retry_count < max_retries:
        ...
        retry_request = ExamRequest(
            ...
            total_questions=exam_request.total_questions * (retry_count + 2),  # Increase multiplier with each retry
    ...
    logger.info(f"Exam Response: {exam_response}")
