...
def generate_mcqs_with_mistral(context: str, exam_request: ExamRequest):
    try:
        ...
        # Use a simpler prompt that matches what Mistral seems to produce naturally
        prompt = f"""
Generate {exam_request.total_questions} multiple-choice questions about the following medical text. 
Each question should include:
1. The question text
2. A list of possible answers (at least 2 options)
...
"""
        ...
        logger.info(f"Successfully processed {len(validated_questions)} questions")
...
async def generate_exam(exam_request: ExamRequest):
    logger.info(f"Generating exam: {exam_request.exam_name} for book: {exam_request.book_name}")
