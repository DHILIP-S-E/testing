        ...
        if state["medi_ai_mode"]:
            ...
            try:
                ...
                if context and "No relevant information" not in context:
                    ...
                    final_answer = "I am unable to find any information about medications for improving eye vision in the current medical eBook sources. It is recommended to consult with a healthcare professional for proper diagnosis and treatment."
            ...
            # MediAI mode is OFF. Respond based on medical_mode.
            if state["medical_mode"]:
                logger.info("MediAI mode is OFF. Medical mode is ON. Responding from MediAI model with Qdrant context.")
                ...
                if not final_answer: # Only call LLM if no direct answer from Qdrant search failure
                    try:
                        response = call_ollama_with_retry(messages_to_send)
                        final_answer = response.choices[0].message.content
                        if "No relevant information" not in context and "[" not in final_answer and "]" not in final_answer:
                        ...
                        final_answer = f"Error generating MediAI response: {str(e)}"
            else:
                logger.info("MediAI mode is OFF. Medical mode is OFF. Responding from Mistral (general purpose) model.")
                ...
                try:
                    ...
                    final_answer = f"Error generating Mistral response: {str(e)}"
